...
from plot_utils import FW, FH, setup_theme, COLORS, LABELS, MARKERS, BIN_PALETTE, TOM_BINS, save_evidence
from spectrum_utils import load_cross_spectrum, compute_snr
...
spectra_files = [Path(p) for p in snakemake.input.spectra]
...
bin_results = {}
spectra_map = {p.name: p for p in spectra_files}
for bid in bins:
    bin_results[bid] = {}
    for cmbk in cmbk_keys:
        npz_path = spectra_map.get(f"{method}_bin{bid}_x_{cmbk}_cls.npz")
        if npz_path is None:
            snakemake_log(snakemake, f"  WARNING: missing {method}_bin{bid}_x_{cmbk}_cls.npz")
            continue
...
fig, axes = plt.subplots(2, 3, figsize=(2.5*FW, 2*FH), sharex=True, sharey=True)
