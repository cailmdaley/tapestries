...
from plot_utils import FW, FH, setup_theme, COLORS, LABELS, BIN_PALETTE, TOM_BINS, save_evidence
...
path_re = re.compile(r"^(?P<method>[a-z0-9_]+)_bin(?P<bin>[a-z0-9_]+)_x_(?P<cmbk>[a-z0-9_]+)_cls\.npz$")
for npz_path in spectra_files:
    m = path_re.match(npz_path.name)
    if m is None:
        continue
    ...
    cmbk = m.group("cmbk")
    key = f"{method}_{cmbk}"
    if key not in ratios_by_cmbk or bin_id not in TOM_BINS:
        continue
...
evidence_data = {
    ...
    "input": {"spectra": [str(p) for p in spectra_files]},
